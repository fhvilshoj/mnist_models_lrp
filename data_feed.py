...
def fetch_data():
    ...
    if not exists('%s/%s' % (data_dir, data_file)):
        ...
        # Scale data do be between -1 and 1
        train_data = mnist.train.images * 2 - 1
        validation_data = mnist.validation.images * 2 - 1
        test_data = mnist.test.images * 2 - 1
        ...
        mnist_data = np.load('%s/%s' % (data_dir, data_file))
